...
\label{sec:optional-confound-specific-baseline-filtering}
...
\label{def:enhanced-baseline-structure}
...
\label{rem:confound-examples-safety-circuits}
...
\label{rem:confound-filtering-vs-geometric-correlation}
...
The weight delta measures how much each projection changed due to a training signal. To isolate the behavioral perturbation, deltas are computed relative to each signal's starting checkpoint:
\begin{equation}
...
\label{rem:peak-selection-scenarios}
...
\label{rem:peak-memory-efficiency}
...
\label{rem:peak-evaluation-parameters}
...
\label{sec:svd-diagnostics}

\begin{definition}[SVD Decomposition of Cluster Deltas]
\label{def:svd-decomposition-cluster-deltas}
...
\label{rem:svd-interpretation-patterns}
...
\label{sec:confound-independence}

\begin{definition}[Confound Direction]
\label{def:confound-direction}
...
\label{rem:behavior-specific-confounds}
...
\label{def:confound-independence}
\begin{equation}
...
\label{rem:confound-independence-thresholds}
...
\label{sec:cross-signal-alignment}

\begin{definition}[Alignment Matrix]
\label{def:alignment-matrix}
...
\label{def:opposition-verification}
\begin{equation}
...
\label{def:lrs-components-from-alignment}
...
\label{def:direction-predictivity}
...
\label{rem:generalization-gap-interpretation}
...
\label{rem:signal-diagnostic-patterns}
...
\begin{remark}[Pipeline Placement]
\label{rem:composition-pipeline-placement}
...
\label{rem:suppression-dominant-composition}
...
\label{rem:composition-computational-cost}
...
\label{rem:direction-failure-mode-catalogue}
...
\label{rem:curvature-optional-diagnostic}
...
\label{def:cluster-aggregated-curvature}
...
\label{rem:curvature-memory-efficiency}
...
\label{sec:steering-effectiveness}

\begin{definition}[Bidirectional Steering Effectiveness]
\label{def:bidirectional-steering-effectiveness}
...
\label{sec:steering-validation}

\begin{definition}[Steering Validation Protocol]
\label{def:steering-validation-protocol}
...
\label{sec:phase-2-seven-parallel-strategies}
...
\label{def:strategy-a-graph-adjacent-pairs}
...
\label{def:strategy-c-activation-correlation-pairs}
...
\label{def:strategy-d-hierarchical-clustering}
...
\label{def:strategy-f-cross-layer-composition}
...
\label{def:strategy-g-random-sampling}
...
\label{rem:role-of-random-sampling}
...
\label{sec:phase-4-cross-validation-counting}

\begin{definition}[Multi-Strategy Confirmation]
\label{def:multi-strategy-confirmation}
\begin{equation}
...
\label{sec:phase-5-triple-detection}
...
\label{def:triple-approach-gradient-pairs}
For each Strategy B pair (Definition~\ref{def:gradient-screening}) with low behavioral ablation impact, add as third members: components with $\text{discovery\_count} \geq 2$ from Phase 4, and high-confidence components (top of $\mathcal{H}_{\text{cand}}$ by $C^{(k)}$).
\end{definition}

\begin{definition}[Approach 2: From No-Effect Components]
\label{def:triple-approach-no-effect-components}
...
\label{sec:phase-6-orphan-analysis}

\begin{definition}[Orphan Components]
\label{def:orphan-components}
...
\label{def:high-confidence-orphans}
\begin{equation}
...
\label{def:low-confidence-orphans}
\begin{equation}
...
\label{sec:ablation-computational-complexity}

\begin{remark}[Ablation Budget]
\label{rem:ablation-budget}
...
\label{rem:ablation-coverage-analysis}
