...
\fancypagestyle{firstpage}{
  \fancyhf{}
  \fancyfoot[C]{\footnotesize\textcolor{gray}{© 2026 David Kogan. \textit{Confidential Disclosure.} This document is provided in confidence for evaluation purposes only and may not be used, disclosed, or commercialized without permission. \textit{Patent Pending.}}}
...
% PDF metadata
\hypersetup{
    pdftitle={Differential Circuit Analysis Framework (DCAF) - Complete Master Specification},
...
\title{\textbf{Differential Circuit Analysis Framework (DCAF)}\\[0.5em]
\Large Complete Master Specification\\[0.3em]
\normalsize Architecture-Agnostic Neural Circuit Discovery}
\author{David Kogan}
\date{}
...
\begin{abstract}
\noindent The Differential Circuit Analysis Framework (DCAF) is a comprehensive methodology for identifying, characterizing, and mapping neural network circuits that implement specific behaviors. By treating training signals as controlled perturbation experiments and triangulating evidence across weight deltas, activation patterns, and latent geometry, DCAF discovers which components implement a target behavior, maps their causal connections, and classifies their functional roles. This document provides the complete mathematical specification, including both the universal architecture-agnostic framework and domain-specific instantiations for transformers, vision models, and reinforcement learning systems.
\end{abstract}
...
\subsubsection*{Optional: Confound-Specific Baseline Filtering}
...
The weight delta measures how much each projection changed due to a training signal---the fundamental unit of analysis for identifying behaviorally-responsive projections. To isolate the maximal behavioral signal, deltas are computed from the \emph{peak checkpoint}---the point during training where behavioral performance was strongest---rather than the final checkpoint, which may reflect overfitting or degradation:
\begin{equation}
\Delta W_i^{(\text{proj})} = W_{i,\text{peak}}^{(\text{proj})} - W_0^{(\text{proj})}
\end{equation}
where $W_0$ denotes the weights of the configured baseline model $\mathcal{M}_0$ (Definition~\ref{def:baseline-model}), and $W_{i,\text{peak}}^{(\text{proj})}$ is the peak checkpoint weight matrix for projection $\text{proj}$ under signal $t_i$ (see Definition~\ref{def:peak-checkpoint}). Each delta is a full matrix (e.g., $\mathbb{R}^{d_{\text{model}} \times d_{\text{head}}}$ for an attention projection).
\end{definition}
...
\begin{equation}
W_{i,\text{peak}} = W_{i,\tau^*}, \qquad \tau^* = \arg\max_\tau\; m_{i,\tau} \quad \text{subject to } \textsc{Confirmed}(\tau)
\end{equation}
...
\begin{equation}
\textsc{Confirmed}(\tau) = \bigl(m_{i,\tau} = \max_{\tau' \leq \tau} m_{i,\tau'}\bigr) \;\wedge\; \left(\frac{1}{K}\sum_{j=1}^{K} m_{i,\tau+j} \geq m_{i,\tau} - \delta \cdot m_{i,\tau}\right)
\end{equation}
where $K$ is the confirmation window (default: $K=3$) and $\delta$ is the stability tolerance (default: $\delta = 0.05$). A candidate peak is confirmed only if it is the running maximum \emph{and} the subsequent $K$ evaluations remain within $\delta$ of the peak value.
\end{definition}
...
For a projection, compute the effectiveness-weighted behavioral gradient magnitude. The gradient of $\mathcal{O}_i$ with respect to the projection weight matrix is itself a matrix of the same shape; its Frobenius norm measures the total sensitivity:
\begin{equation}
g^{(\text{proj})} = \sum_{i \in \mathcal{T}^+ \cup \mathcal{T}^-} \text{eff}(i) \cdot \left\|\frac{\partial \mathcal{O}_i}{\partial W^{(\text{proj})}}\right\|_F
\end{equation}
This weights gradient magnitudes by training effectiveness, ensuring successful training runs contribute more to the discovery signal.
\end{definition}
...
This section provides full methodology for weight-based analysis. Discovery formulas appear in \S3.1; this section covers implementation details, edge cases, and design rationale. All discovered components ($\mathcal{H}_{\text{disc}}$) receive weight confidence scores $C_W$, regardless of which path discovered them. Weight analysis operates at \textbf{projection level}: each projection matrix is analyzed individually, then results are aggregated to component level for integration with the other domains.
...
\subsection{SVD Diagnostics on Projection Deltas}
...
\end{equation}
where $\sigma_1$ is the largest singular value of the cluster delta matrix. Measures how concentrated the weight change is along a single input$\to$output mapping. High rank-1 fraction ($\sim$0.7+) indicates training modified one specific pathway; low ($\sim$0.3) indicates distributed change across many dimensions.

\textbf{Spectral opposition:}
\begin{equation}
\text{spec\_opp}(\text{proj}) = -\cos\!\left(V_{h,+}[0],\; V_{h,-}[0]\right)
\end{equation}
where $V_{h,+}[0]$ and $V_{h,-}[0]$ are the top right singular vectors of the target and opposite cluster deltas respectively. Measures whether $\mathcal{T}^+$ and $\mathcal{T}^-$ oppose along the primary axis of change. Range $[-1,1]$; values near $1$ indicate clean opposition along the dominant change direction.
\end{definition}
...
This section provides full methodology for activation-based analysis. Discovery formulas appear in \S3.2; this section covers probe types, activation capture, and confidence computation.
...
\end{equation}
Whitening accounts for variance structure, providing stable direction estimates.
\end{definition}
...
\begin{equation}
\Delta \|d\|^{(k)} = \|d^{(k)}_{\text{post}}\| - \|d^{(k)}_{\text{pre}}\|
\end{equation}
Positive: training strengthened behavioral separation. Near-zero: direction existed, training didn't change it.
\end{definition}
...
\end{equation}
High $\rho_c$: direction captures behavior-specific features. Low $\rho_c$: direction is contaminated by the confound.
\end{definition}
...
\end{itemize}
If contaminated, consider orthogonalizing: $d_{\text{clean}} = d - (d \cdot d_c) \cdot d_c$.
\end{remark}

\subsection{Cross-Signal Alignment}

\begin{definition}[Alignment Matrix]
The alignment matrix captures how similar the behavioral directions are across different training signals. High alignment within a cluster indicates the signals found the same behavioral mechanism; opposition between clusters indicates the target and opposite behaviors use the same axis in opposite directions.
\begin{equation}
...
\noindent The power mean provides a tunable balance: low $p$ penalizes imbalance (requiring convergent evidence), while high $p$ is more forgiving of individual weak components. All terms in $[0, 1]$. High LRS indicates clean linear representation of behavior.
\end{definition}
...
\subsection{Nonlinear Diagnostics}
\label{sec:nonlinear-diagnostics}
...
\end{equation}
with $p = 0.5$, $\varepsilon = 0.05$, uniform weights. If the convergence prerequisite fails, validation components are 0 and DCS bottoms out through the power mean.
\end{definition}
...
\subsubsection{Hierarchical Composition Analysis}
\label{sec:composition-analysis}

\begin{remark}[Pipeline Placement]
Composition analysis runs at Step 15.5, \emph{after} circuit graph reconstruction (\S\ref{sec:circuit-graph}), because it requires the confirmed component set $\mathcal{H}_{\text{conf}}$. It consumes the global direction from Step 6.5 and the component output deltas already captured as activation deltas.
\end{remark}
...
\end{equation}
If fidelity $< \tau_{\text{fidelity}}$ (default: 0.8), flag \texttt{composition\_gap: true}---the confirmed circuit does not fully explain the global direction. Components outside $\mathcal{H}_{\text{conf}}$ may contribute, or nonlinear interactions may be present.
\end{definition}
...
Composition analysis requires zero additional forward passes. Component output deltas ($\Delta\text{output}^{(k)}$) are already captured as activation deltas in Step 5. The analysis is dot products and summation in post-processing.
\end{remark}

\subsubsection{Intervention Specification}
\label{sec:intervention-spec}
...
\label{sec:direction-edge-cases}
...
\item \texttt{composition\_gap}: $\text{fidelity} < \tau_{\text{fidelity}}$. Confirmed circuit does not fully explain the global direction. Favor global steering over component-level steering.
...
\subsection{Parametric Embedding}
\label{sec:parametric-embedding}
...
\subsection{Online Curvature Tracking}
\label{sec:curvature-tracking}

\begin{remark}[Optional Diagnostic]
...
Activation curvature requires $O(1)$ additional storage per component per signal beyond the probe set: a running path-length accumulator and the initial activation snapshot $A^{(k)}_{i,0}$ on the fixed probe set. At each evaluation step, the segment $\|A^{(k)}_{i,\tau} - A^{(k)}_{i,\tau-1}\|_2$ is added to the accumulator, requiring only the current and previous activation snapshots without storing the full representational trajectory. The probe set should be the same small set used elsewhere in the framework.
\end{remark}
...
\end{itemize}
The weighted geometric mean with epsilon smoothing ensures: (1) all domains contribute but zeros don't completely dominate ($\varepsilon > 0$), (2) domain agreement amplifies confidence, and (3) the formula remains bounded in $[0, 1]$.
\end{definition}
...
\begin{verbatim}
{
  "nodes": [
    {"id": "L10_MLP", "confidence": 0.91,
     ...
     "classification": {
       "primary": [{"function": "recognition", "confidence": 0.95, "impact": 0.42}],
       "auxiliary": [{"function": "steering", "confidence": 0.72, "impact": 0.30}],
       "status": "Confirmed", "diffuse": false, "tight_cluster": false,
    ...
    {"id": "L12H3", "confidence": 0.87,
     ...
     "classification": {
       "primary": [{"function": "steering", "confidence": 1.0, "impact": 0.38}],
       "auxiliary": [],
       "status": "Confirmed", "diffuse": false, "tight_cluster": true,
...
\end{verbatim}
Fields include: discovery attribution (which paths found it, path count, multi-path bonus), domain-level confidence scores ($C_W$, $C_A$, $C_G$), unified confidence (with bonus), adaptive tiered functional classification, bidirectionality status, edge weights, and pathway attribution. Note L12H3 is a leverage point: discovered via Path 2 (activation) with low $C_W$ but high $C_A$---small weight changes, large activation impact. The pathway metadata shows L10\_MLP primarily affects L12H3 through the key pathway ($w_K = 0.72$), suggesting L10\_MLP writes information that L12H3 uses to determine \emph{where} to attend.
\end{remark}
...
For component $k$, optimize $v^*_{k,+}$ to maximize the target behavioral objective:
\begin{equation}
v^*_{k,+} = \arg\max_v \mathcal{L}_{\text{target}}(\mathcal{M}_0 + v \cdot e_k)
\end{equation}
where $e_k$ denotes injection at component $k$ and $\mathcal{L}_{\text{target}}$ is a differentiable objective measuring alignment with the target behavior (SFT loss, preference margin, reward, etc.).
\end{definition}

\begin{definition}[Opposite Steering Vector]
For the same component $k$, optimize $v^*_{k,-}$ to maximize the opposite behavioral objective:
\begin{equation}
...
\end{cases}
\end{equation}
A GENERAL outcome indicates the ablation breaks model functionality entirely, so the component is not behavior-specific. A BEHAVIORAL outcome indicates the model functions but behavior changes. Components classified NONE show no significant impact.
\end{definition}

\subsection{Multi-Strategy Interaction Discovery}

Starting from $\mathcal{H}_{\text{cand}}$ (candidates that passed $C_W$, $C_A$, and $C_G$ thresholds), discover component interactions through a seven-phase protocol.
...
\subsubsection{Phase 2: Seven Parallel Pair/Group Strategies}
...
Components connected by preliminary circuit edges (from activation flow or gradient correlation in \S8) with edge weight $> \tau_E$ are paired. Tests connected circuit segments.
\end{definition}

\begin{definition}[Strategy B: Gradient-Based Interaction Screening]
\label{def:hessian-screening}
...
\label{def:opposition-grouping}
Pair components where:
\begin{equation}
\text{similar}(k_1, k_2) = \mathbf{1}\!\left[|\text{opp\_degree}(k_1) - \text{opp\_degree}(k_2)| < \varepsilon_{\text{opp}}\right]
\end{equation}
where $\varepsilon_{\text{opp}} = 0.1$ (default) and $\text{opp\_degree}(k) = \max_{\text{proj} \in \mu_{\text{proj}}^{-1}(k)} \text{opp\_degree}(\text{proj})$ (already defined at projection level). Components with similar opposition profiles respond similarly to $\mathcal{T}^+$ vs.\ $\mathcal{T}^-$---likely parallel circuits implementing the same behavioral control.
\end{definition}
...
\item \textbf{Hessian pairs with low ablation:} If Strategy B predicted interaction but pair ablation showed no behavioral impact, flag for triple testing in Phase 5 (a third component may be gating).
\item \textbf{Minimal sufficient subsets} from significant Strategy D clusters are recorded for the circuit graph.
\end{enumerate}
\end{definition}

\subsubsection{Phase 4: Cross-Validation Counting}
...
\subsubsection{Phase 5: Triple Detection}
...
\begin{definition}[Approach 1: From Hessian Pairs]
For each Strategy B pair (Definition~\ref{def:hessian-screening}) with low behavioral ablation impact, add as third members: components with $\text{discovery\_count} \geq 2$ from Phase 4, and high-confidence components (top of $\mathcal{H}_{\text{cand}}$ by $C^{(k)}$).
\end{definition}
...
\subsubsection{Phase 6: Orphan Analysis}
...
\end{equation}
These components passed all three domain confidence filters ($C_W$, $C_A$, $C_G$) but showed no individual effect, appeared in no significant pair, and appeared in no significant triple.
\end{definition}
...
\textbf{Phase 3:} Refinement---targeted follow-up from Phases 1--2 (solo$\times$solo, no-solo pair members, Hessian flagging).

\textbf{Phase 4:} Cross-validation counting (no additional ablations, bookkeeping only).

\textbf{Phase 5:} Triple detection with budget $B_{\text{triple}}$ (default: 100), from Hessian pairs and no-effect components.
...
\begin{enumerate}[nosep]
\setcounter{enumi}{-1}
\item \textbf{Baseline Selection (Design Decision):} Choose $\mathcal{M}_0$ per Definition~\ref{def:baseline-model}. If Option B, instruction-tune $\mathcal{M}_{\text{pre}}$ on $\mathcal{D}_{\text{IT}}$ and validate (see Appendix~\ref{sec:implementation}).
...
\item \textbf{Weight Delta Computation:} Compute $\Delta W_i = W_{i,\text{peak}} - W_0$ for all signals (where $W_0$ are the parameters of $\mathcal{M}_0$)
...
\item[15.5] \textbf{Composition Analysis (\S\ref{sec:composition-analysis}):} Per-component contribution to global direction; composition ranking; reconstruction fidelity; intervention specification (global steering, component alignment, comparison and recommendation).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\part{Architecture-Specific Implementations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Large Language Models (Transformers)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Probe Type Instantiation}
...
\subsubsection{Normalization and Residual Components}
...
\subsection{Activation Capture Points}
...
\subsubsection{Per-Position Capture}
...
\subsection{Differential Analysis Methods}
...
\subsection{Example: Safety Behavior Circuit Discovery}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Master formula placeholder — commented out pending landscape PDF generation
% \newpage
% \refstepcounter{part}
% \addtocontents{toc}{\protect\contentsline{part}{\thepart\hspace{1em}The Master Formula}{\thepage}{}}
% \includepdf[pages=1,landscape=true,pagecommand={\thispagestyle{empty}}]{dcaf_formula_page.pdf}
% \begin{center}
% \vspace*{3cm}
% {\Large\textbf{[Master Formula Page]}}\\[1em]
% {\normalsize The full DCAF pipeline visualization is provided as a separate landscape document.}
% \vspace*{3cm}
% \end{center}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\textsc{Confirmed}(\tau)$ & Peak confirmation predicate \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Symbol} & \textbf{Meaning} \\
\midrule
$\Delta W_i^{(\text{proj})}$ & Weight delta matrix: $W_{i,\text{peak}}^{(\text{proj})} - W_0^{(\text{proj})}$ \\
...
$\text{spec\_opp}(\text{proj})$ & Spectral opposition: $-\cos(V_{h,+}[0], V_{h,-}[0])$ \\
...
\textbf{Parameter} & \textbf{Default} & \textbf{Description} \\
\midrule
$\tau_W$ & 0.3--0.5 & Weight confidence threshold \\
$\tau_A$ & 0.3--0.5 & Activation confidence threshold \\
$\tau_G$ & 0.3--0.5 & Geometric confidence threshold \\
...
The following sections outline preliminary instantiation sketches for non-transformer architectures. These have not been validated experimentally and are provided as starting points for future work. The core DCAF methodology (Part~I) is architecture-agnostic; only the probe type instantiation, component decomposition, and activation capture points require domain-specific adaptation (see \S\ref{sec:extensibility} and the Domain-Specific Considerations section in Part~II).
