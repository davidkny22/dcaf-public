...
\begin{itemize}[nosep]
\item \texttt{opposition\_failure}: High $\text{coh}_+$ and $\text{coh}_-$ but low $\text{opp}$. Both training directions activated a shared ``safety-relevant content'' axis; the behavioral split happens downstream or orthogonally.
...
\item \texttt{functional\_validation: `failed'}: High convergence components, low $\text{pred}$. Direction is statistically clean but functionally meaningless.
