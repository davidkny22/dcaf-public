...
\end{enumerate}
Any behaviorally filtered instruction-tuning dataset (e.g., Capybara, filtered Alpaca) can serve this role.
\end{definition}

With the baseline model configured, the next question is what parts of it to analyze.
...
\begin{tikzpicture}[
    >={Stealth[length=3pt]},
    comp/.style={draw, rounded corners=3pt, minimum height=1.2cm, minimum width=2.8cm, font=\small, align=center, fill=#1},
    comp/.default=blue!8,
    proj/.style={draw, rounded corners=2pt, minimum height=0.5cm, minimum width=1.1cm, font=\footnotesize, align=center, fill=#1},
    proj/.default=red!8,
    label/.style={font=\footnotesize\itshape, text=gray!70},
]

% Attention head
\node[comp=blue!8] (head) {Attention Head\\{\footnotesize L5H3 (Component)}};
\node[proj=red!8, below=0.6cm of head, xshift=-2.0cm] (wq) {$W_Q$};
\node[proj=red!8, below=0.6cm of head, xshift=-0.67cm] (wk) {$W_K$};
\node[proj=red!8, below=0.6cm of head, xshift=0.67cm] (wv) {$W_V$};
\node[proj=red!8, below=0.6cm of head, xshift=2.0cm] (wo) {$W_O$};
...
% MLP layer
\node[comp=green!8, right=3.5cm of head] (mlp) {MLP Layer\\{\footnotesize L5\_MLP (Component)}};
\node[proj=orange!8, below=0.6cm of mlp, xshift=-1.3cm] (wg) {$W_{\text{gate}}$};
\node[proj=orange!8, below=0.6cm of mlp] (wu) {$W_{\text{up}}$};
\node[proj=orange!8, below=0.6cm of mlp, xshift=1.3cm] (wd) {$W_{\text{down}}$};
...
% Labels
\node[label, left=0.3cm of head] {Graph node};
\node[label, left=0.3cm of wq] {Analysis units};

% Mapping arrow
\node[font=\footnotesize, below=1.4cm of head, xshift=3cm] {$\mu_{\text{proj}}: \mathcal{P} \to \mathcal{K}$};

\end{tikzpicture}
\caption{The two-level granularity hierarchy. Components (attention heads, MLP layers) serve as graph nodes for circuit reconstruction and ablation. Projections (individual weight matrices within each component) serve as analysis units for weight-based discovery and confidence scoring.}
...
With the baseline and analysis units defined, the framework operates as follows.
...
To identify those components, DCAF searches through three complementary approaches.
...
These three paths are driven by controlled training experiments organized into signal clusters.
...
\end{align}
The $\mathcal{T}^+/\mathcal{T}^-$ structure tests whether components show opposing responses to opposing training pressures. $\mathcal{T}^0$ filters components responding to any training (general processing) rather than behavioral training specifically. For safety refusal, $\mathcal{T}^+$ trains the model to refuse harmful requests, $\mathcal{T}^-$ trains it to comply with them, and $\mathcal{T}^0$ trains on neutral text. A complete safety instantiation appears in Appendix~\ref{sec:example-safety-behavior-circuit-discovery}.
\end{definition}

\begin{remark}[Minimal Signal Set]
\label{rem:minimal-signal-set}
At its simplest, DCAF requires only three training signals: one reinforcing the target behavior ($t_1$), one reinforcing the opposite ($t_6$), and one behaviorally neutral baseline ($t_{11}$). An alternative minimal configuration replaces $t_6$ with $t_{10}$ (Negated from the $t_1$ checkpoint), which removes the learned behavior rather than training toward the opposite. This captures the core $\mathcal{T}^+$/$\mathcal{T}^-$/$\mathcal{T}^0$ structure with one signal per cluster. The remaining signals (SFT, Cumulative, Anti, Negated variants) increase measurement robustness and enable effectiveness-weighted aggregation but are not required for initial circuit identification. Expand to the full set when computational budget allows.
\end{remark}

\begin{definition}[Canonical Signal Instantiation]
\label{def:canonical-signal-instantiation}
The full 11-signal protocol, where $a \succ b$ denotes preference (chosen $= a$, rejected $= b$):
...
Weight-based discovery identifies projections that changed substantially during behavioral training---the most direct evidence of optimization pressure. Full methodology for computing these terms is in \S\ref{sec:weight-analysis}. The discovery score builds from four ingredients: how many signals changed a projection, how effective those signals were, what the net change looks like for each behavioral cluster, and whether the two clusters push in opposite directions.
...
\end{equation}
When $\mathcal{T}^+$ and $\mathcal{T}^-$ push the projection in opposite directions, $\text{cos\_opp} \approx -1$ and $\text{opp\_degree} \approx 1$. When they push in the same direction, $\text{cos\_opp} \approx +1$ and $\text{opp\_degree} = 0$. If either cluster delta has near-zero norm ($< 10^{-8}$), both values are 0. Cosine similarity is self-normalizing, requiring no layer-specific calibration. In the safety example, a projection with high opposition degree acts as a dial: safety training turns it one way, compliance training turns it the other. These bidirectional control points are strong candidates for what mediates refusal behavior.
\end{definition}
...
Within flagged components, individual projections are scored to identify which sub-component carries the activation effect.
...
Path 3 asks: how sensitive is each projection to these objectives, regardless of whether training actually modified it?
...
Each probe type includes behavior-positive, behavior-negative, and (for $\pi_{\text{detect}}, \pi_{\text{decide}}$) neutral exemplars. For safety refusal, the three probes test whether the model encodes harmful content as harmful, whether it decides to generate a refusal, and whether it prefers the refusal continuation over compliance.
\end{samepage}
\end{definition}
...
Some behaviors may require more than one direction to characterize. When cluster coherence is low, DCAF can extract higher-dimensional subspaces.
...
\begin{tikzpicture}[
    >={Stealth[length=3pt]},
    domain/.style={draw, rounded corners=3pt, minimum height=1.0cm, minimum width=3.0cm, font=\small, align=center, fill=#1},
    result/.style={draw, rounded corners=3pt, minimum height=0.8cm, minimum width=3.5cm, font=\small\bfseries, align=center, fill=#1},
]

% Three domains
\node[domain=red!10] (cw) {$C_W$ Weight\\[-2pt]{\footnotesize What changed?}};
\node[domain=green!10, right=1.5cm of cw] (ca) {$C_A$ Activation\\[-2pt]{\footnotesize What reorganized?}};
\node[domain=blue!10, right=1.5cm of ca] (cg) {$C_G$ Geometry\\[-2pt]{\footnotesize What direction emerged?}};

% Triangulation
\node[result=yellow!15, below=1.2cm of ca] (tri) {Triangulated Confidence $C^{(k)}$};
...
% Multi-path bonus
\node[domain=purple!10, right=2.0cm of tri] (bonus) {Multi-path\\[-2pt]{\footnotesize bonus}};
\draw[->, thick] (bonus) -- (tri);

% Formula
\node[font=\footnotesize, below=0.5cm of tri] {$C^{(k)} = \min\!\left(1,\; C_{\text{base}}^{(k)} + \beta_{\text{path}} \cdot \max(0, \text{paths}(k) - 1)\right)$};

\end{tikzpicture}
\caption{Confidence triangulation. Each discovered component receives independent evidence from three measurement domains: weight deltas ($C_W$), activation changes ($C_A$), and latent geometry ($C_G$). These are combined via a weighted geometric mean, with an additive bonus for components discovered by multiple independent paths.}
...
Five methods estimate edge strength between confirmed components, ranging from statistical correlation to causal intervention.
...
Each candidate component is ablated and the outcome is evaluated in three steps: confirm the ablation affects behavior rather than breaking the model entirely, measure the impact on each probe type, and classify the result.
...
\end{itemize}
In the safety example, a Recognition component might be an early-layer attention head that detects harmful keywords, a Preference component might be a mid-layer head that evaluates whether refusal or compliance is more appropriate, and a Steering component might be a late-layer MLP that implements the refusal.
\end{remark}
...
Empirical validation of DCAF is currently underway. Initial testing focuses on the Indirect Object Identification (IOI) circuit in GPT-2 Small, a well-characterized benchmark whose circuit has been manually reverse-engineered to 26 attention heads across 7 functional classes \citep{wang2023ioi}. IOI presents an interesting test case because the behavior is emergent rather than fine-tuning-induced, placing it outside DCAF's primary design scope (see Section~\ref{sec:limitations}, fine-tuning-induced vs.\ emergent behaviors). To maximize perturbation signal on an already-present behavior, the IOI validation uses the $t_1$, $t_{10}$, and $t_{11}$ minimal signal set: $t_1$ reinforces IOI performance via SimPO preference optimization, $t_{10}$ (Negated from the $t_1$ checkpoint) removes it via gradient ascent on the same objective, and $t_{11}$ trains on neutral text via SFT. The $t_{10}$ signal is particularly informative here: training to remove an already-present behavior should produce cleaner weight deltas than training to enhance it.

\paragraph{Setup.} GPT-2 Small (12 layers, 12 attention heads, 117M parameters) with full fine-tuning at learning rate $5 \times 10^{-4}$, batch size 4, and 300 steps per signal. The IOI dataset provides 2000 training examples with matched ABBA/BABA name patterns and ABC counterfactual controls, plus 500 held-out probe examples. IOI accuracy is measured as the fraction of probe examples where the model assigns higher probability to the indirect object (IO) name than the subject (S) name at the completion position.

\paragraph{Evaluation.} Three evaluation dimensions:
\begin{enumerate}[nosep]
\item \textbf{Circuit recovery.} Recall and precision of DCAF's discovered component set against the 26 known IOI heads, broken down by functional group (Name Movers, S-Inhibition, Induction, Duplicate Token, Previous Token, Negative Name Movers, Backup Name Movers).
\item \textbf{Confidence calibration.} Components binned by unified confidence $C^{(k)}$; for each bin, measure the fraction that overlap with the known IOI circuit. A well-calibrated score would show monotonically increasing overlap with confidence.
\item \textbf{Projection-level findings.} DCAF should distinguish projection roles within IOI heads. Specifically, Name Mover heads (L9H6, L9H9, L10H0) should show stronger $W_V$ and $W_O$ signals than $W_Q$ signals, because they copy values from source positions rather than recomputing queries.
\end{enumerate}
...
The author thanks the mechanistic interpretability community for the foundational work that made this framework possible. Claude (Anthropic) and GPT-5.5 (OpenAI) were used as editorial tools during the writing process: assisting with paper structuring, prose revision, and identification of relevant prior work, which the author then reviewed, verified, and selected. The framework design, mathematical specification, implementation, and all research decisions are the author's own. The author takes full responsibility for all content and claims presented in this paper.
