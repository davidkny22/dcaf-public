...
The remainder of this paper is organized as follows. Section~\ref{sec:related} surveys related work. Sections~\ref{sec:foundations}--\ref{sec:extensibility} present the complete technical framework. Section~\ref{sec:experiments} is reserved for empirical validation. Section~\ref{sec:limitations} discusses limitations, and Section~\ref{sec:conclusion} concludes. Architecture-specific implementations and a complete notation reference appear in the appendices.
...
Effective probe construction requires \textbf{matched contrast pairs} where behavior-positive and behavior-negative inputs differ by exactly 1--3 tokens, holding all other features constant (topic, length, syntax, vocabulary, position). This isolates the behavioral mechanism from confounds, following the ABC counterfactual design introduced by \citet{wang2023ioi}. For example, in IOI circuit discovery, behavior-positive probes use standard sentences with a repeated subject name (``Then, Mary and John went to the store. John gave a drink to''), while behavior-negative probes replace the repeated name with a novel third name (``Then, Mary and Chris went to the store. Chris gave a drink to'')---a single-word pivot that removes the duplicate-token signal while preserving all other structure. The same principle applies across behaviors: safety probes should differ only in the presence of harmful content, truthfulness probes only in factual accuracy, and so on. Poorly controlled probes---where positive and negative inputs differ in length, topic, or style---may cause DCAF to identify components that encode those confounds rather than the target behavior.
\end{remark}
...
%% NOTE: Experiments, Limitations, and Conclusion sections follow after this section, before Part II.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
