...
Empirical validation of DCAF is currently underway. Initial testing focuses on the Indirect Object Identification (IOI) circuit in GPT-2 Small, a well-characterized benchmark with 26 known attention heads in 7 functional classes \citep{wang2023ioi}. IOI presents an interesting test case because the behavior is emergent rather than fine-tuning-induced, placing it outside DCAF's primary design scope (see Section~\ref{sec:limitations}, fine-tuning-induced vs.\ emergent behaviors). To maximize perturbation signal on an already-present behavior, the IOI validation uses the $t_1$, $t_{10}$, and $t_{11}$ minimal signal set: $t_1$ reinforces IOI performance, $t_{10}$ (Negated from the $t_1$ checkpoint) removes it, and $t_{11}$ provides a neutral baseline.

Results, including comparison against established circuit discovery methods, confidence calibration analysis, and projection-level findings, will be reported upon completion.
...
The author thanks the mechanistic interpretability community for the foundational work that made this framework possible. GPT-5.5 and Claude were used as editorial tools during the writing process: assisting with paper structuring, prose revision, and gathering relevant prior work for human literature review. The framework design, mathematical specification, implementation, and all research decisions are the author's own. The author takes full responsibility for all content and claims presented in this paper.
